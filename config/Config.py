...
class Config(object):
    ...
    def init(self):
        ...
        self.origin_h = np.zeros(self.batch_seq_size, dtype=np.int64)
        self.origin_t = np.zeros(self.batch_seq_size, dtype=np.int64)
        self.origin_r = np.zeros(self.batch_seq_size, dtype=np.int64)
        self.batch_y = np.zeros(self.batch_seq_size, dtype=np.float32)
        self.batch_h_addr = self.batch_h.__array_interface__["data"][0]
        self.batch_t_addr = self.batch_t.__array_interface__["data"][0]
        self.batch_r_addr = self.batch_r.__array_interface__["data"][0]
        self.origin_h_addr = self.origin_h.__array_interface__["data"][0]
        self.origin_t_addr = self.origin_t.__array_interface__["data"][0]
        self.origin_r_addr = self.origin_r.__array_interface__["data"][0]
    ...
    def set_rdict_path(self, rdict_path):
        self.rdict_path = rdict_path

    def set_premodel_path(self, premodel_path):
        self.premodel_path = premodel_path
  
    def set_hdict_path(self, hdict_path):
        self.hdict_path = hdict_path

    def set_tdict_path(self, tdict_path):
        self.tdict_path = tdict_path

    def set_pot_mode(self, por_rel_mode):
        self.por_rel_mode = por_rel_mode

    def set_filter_lmbda(self, filter_lmbda):
        self.filter_lmbda = filter_lmbda

    def set_nextStage(self, nextStage):
    ...
    def set_train_model(self, model, path=None):
        ...
        # device_ids = [0,1,2,3] 
        # self.parallModel = nn.DataParallel(self.trainModel, device_ids=device_ids)
        
        # self.parallModel.to(device=device_ids[0])
        ...
        if self.optimizer != None:
            ...
            # )
            # self.optimizer = nn.DataParallel(optimizer, device_ids=[0,1,2,3])
    ...
    def set_test_model(self, model, path=None):
        ...
        # self.testModel.rel_dist = ent_dist
        # self.testModel.pot_rel_dist = pot_rel_dist
        # self.testModel.predict_vector = predict_vec
        self.testModel.cuda()
        self.testModel.eval()
        print("Finish initializing")
    ...
    def train_one_step(self):
        # self.trainModel = self.parallModel.module
        ...
        # self.trainModel.origin_h = to_var(self.origin_h)
        # self.trainModel.origin_t = to_var(self.origin_t)
        # self.trainModel.origin_r = to_var(self.origin_r)
    ...
    def train(self):
        ...
        for epoch in range(self.train_times):
            ...
            if (epoch + 1) == self.nextStage:
                try:
                    self.trainModel.triple_augment()
                except:
                    print("not have triple_augment")
            if (epoch + 1) % self.save_steps == 0:
                # best_loss = res
                print("Epoch %d has finished, saving..." % (epoch))
                self.save_checkpoint(self.trainModel.state_dict(), epoch)
            # if (epoch + 1) % self.valid_steps == 0:
            if (epoch + 1) >= self.valid_steps and (epoch + 1) % 100 == 0:
                ...
                if hit10 > best_hit10:
                    ...
                    # ent_dist = self.trainModel.rel_dist
                    # pot_rel_dist = self.trainModel.pot_rel_dist
                    # predict_vec = self.trainModel.predict_vector
        ...
        print("Testing...")
        self.set_test_model(self.model) # self.model
        self.test()
        print("Finish test")
        return best_model
