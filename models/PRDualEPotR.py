...
class PRDualEPotR(Model):
    def __init__(self, config):
        super(PRDualEPotR, self).__init__(config)
        ...
        self.rel_1 = nn.Embedding(self.config.relTotal * 2, self.config.hidden_size)
        self.rel_2 = nn.Embedding(self.config.relTotal * 2, self.config.hidden_size)
        self.rel_3 = nn.Embedding(self.config.relTotal * 2, self.config.hidden_size)
        self.rel_4 = nn.Embedding(self.config.relTotal * 2, self.config.hidden_size)
        self.rel_5 = nn.Embedding(self.config.relTotal * 2, self.config.hidden_size)
        self.rel_6 = nn.Embedding(self.config.relTotal * 2, self.config.hidden_size)
        self.rel_7 = nn.Embedding(self.config.relTotal * 2, self.config.hidden_size)
        self.rel_8 = nn.Embedding(self.config.relTotal * 2, self.config.hidden_size)
        self.aug_h = []
        self.aug_r = []
        self.aug_ent = {}
    ...
    def init_weights(self):
        if self.config.nextStage == 1:
            ...
            for rel in range(self.config.relTotal):
                self.rel_1.weight.data[rel] = pre_model['rel_1.weight'][rel]
                self.rel_2.weight.data[rel] = pre_model['rel_2.weight'][rel]
                self.rel_3.weight.data[rel] = pre_model['rel_3.weight'][rel]
                self.rel_4.weight.data[rel] = pre_model['rel_4.weight'][rel]
                self.rel_5.weight.data[rel] = pre_model['rel_5.weight'][rel]
                self.rel_6.weight.data[rel] = pre_model['rel_6.weight'][rel]
                self.rel_7.weight.data[rel] = pre_model['rel_7.weight'][rel]
                self.rel_8.weight.data[rel] = pre_model['rel_8.weight'][rel]
                self.rel_1.weight.data[rel+self.config.relTotal] = pre_model['rel_1.weight'][rel]
                self.rel_2.weight.data[rel+self.config.relTotal] = pre_model['rel_2.weight'][rel]
                self.rel_3.weight.data[rel+self.config.relTotal] = pre_model['rel_3.weight'][rel]
                self.rel_4.weight.data[rel+self.config.relTotal] = pre_model['rel_4.weight'][rel]
                self.rel_5.weight.data[rel+self.config.relTotal] = pre_model['rel_5.weight'][rel]
                self.rel_6.weight.data[rel+self.config.relTotal] = pre_model['rel_6.weight'][rel]
                self.rel_7.weight.data[rel+self.config.relTotal] = pre_model['rel_7.weight'][rel]
                self.rel_8.weight.data[rel+self.config.relTotal] = pre_model['rel_8.weight'][rel]
            del pre_model
        else:
    ...
    def forward(self):
        ...
        if self.isAugmented:
            sample_num = random.sample(range(0, len(self.aug_h)), len(self.batch_h) // self.config.nbatches)
            sampled_pseudo_h = self.aug_h[sample_num]
            sampled_pseudo_r = self.aug_r[sample_num]
            regul3, r_regul = self.sampled_pseudo_loss(sampled_pseudo_h, sampled_pseudo_r)
            regul2 += r_regul
    ...
    def triple_augment(self):
        ...
        for rel in range(self.config.relTotal):
            self.rel_1.weight.data[rel+self.config.relTotal] = self.rel_1.weight.data[rel]
            self.rel_2.weight.data[rel+self.config.relTotal] = self.rel_2.weight.data[rel]
            self.rel_3.weight.data[rel+self.config.relTotal] = self.rel_3.weight.data[rel]
            self.rel_4.weight.data[rel+self.config.relTotal] = self.rel_4.weight.data[rel]
            self.rel_5.weight.data[rel+self.config.relTotal] = self.rel_5.weight.data[rel]
            self.rel_6.weight.data[rel+self.config.relTotal] = self.rel_6.weight.data[rel]
            self.rel_7.weight.data[rel+self.config.relTotal] = self.rel_7.weight.data[rel]
            self.rel_8.weight.data[rel+self.config.relTotal] = self.rel_8.weight.data[rel]
        for dh in tqdm(range(self.config.entTotal)):
            selected_r = self.topK_ent_rel(dh)
            if dh in self.h_rel_dict:
                for dr in selected_r:
                    if dr not in self.h_rel_dict[dh]:
                        self.aug_h.append(dh)
                        self.aug_r.append(dr)
                        self.pred_ent_t(dh, dr)
            else:
                for dr in selected_r:
                    self.aug_h.append(dh)
                    self.aug_r.append(dr)
                    self.pred_ent_t(dh, dr)
        print("Add triples: ", len(self.aug_h))
        self.aug_h = torch.from_numpy(np.array(self.aug_h))
        self.aug_h = self.aug_h.cuda()
        self.aug_r = torch.from_numpy(np.array(self.aug_r))
        self.aug_r = self.aug_r.cuda()
    ...
    def sampled_pseudo_loss(self, sampled_pseudo_h, sampled_pseudo_r):
        ...
        r_1 = self.rel_1(sampled_pseudo_r + self.config.relTotal)
        r_2 = self.rel_2(sampled_pseudo_r + self.config.relTotal)
        r_3 = self.rel_3(sampled_pseudo_r + self.config.relTotal)
        r_4 = self.rel_4(sampled_pseudo_r + self.config.relTotal)
        r_5 = self.rel_5(sampled_pseudo_r + self.config.relTotal)
        r_6 = self.rel_6(sampled_pseudo_r + self.config.relTotal)
        r_7 = self.rel_7(sampled_pseudo_r + self.config.relTotal)
        r_8 = self.rel_8(sampled_pseudo_r + self.config.relTotal)
    ...
    def topK_ent_rel(self, dh):
        with torch.no_grad():
            ...
            for h_i in indices.tolist():
                nob_rels.update(tuple(self.h_rel_dict[h_i]))
        return list(nob_rels)

    def pred_ent_t(self, dh, dr):
        with torch.no_grad():
            key = str(dh) + "_" + str(dr)
